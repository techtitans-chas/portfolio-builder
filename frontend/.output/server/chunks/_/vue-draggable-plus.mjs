...
var ae = re(), xe = re(), It = re(), Te = re() && !re() && !re(), Lt = re(), Wt = re() && re(), Gt = {
  capture: false,
  passive: false
};
function S(t, e, n) {
...
function to(t) {
  vueExports.getCurrentInstance() && vueExports.onUnmounted(t);
}
function no(t) {
  vueExports.getCurrentInstance() ? vueExports.onMounted(t) : vueExports.nextTick(t);
}
let tn = null, nn = null;
...
function on(...t) {
  var le, k;
  const e = (le = vueExports.getCurrentInstance()) == null ? void 0 : le.proxy;
  let n = null;
  const o = t[0];
  let [, r, i] = t;
  Array.isArray(vueExports.unref(r)) || (i = r, r = null);
  let a = null;
  const {
    immediate: l = true,
    clone: s = eo,
    ...
    fallbackOnBody: d,
    customUpdate: f
  } = (k = vueExports.unref(i)) != null ? k : {};
  function m(v) {
    var F;
    const { from: D, oldIndex: A, item: I } = v, _ = Array.from(D.childNodes);
    n = u && !d ? _.slice(0, -1) : _;
    const x = vueExports.unref((F = vueExports.unref(r)) == null ? void 0 : F[A]), M = s(x);
    Xt(x, M), I[Yt] = M;
  }
  function y(v) {
    const D = v.item[Yt];
    if (!wn(D)) {
      if (Ke(v.item), vueExports.isRef(r)) {
        const A = [...vueExports.unref(r)];
        r.value = _t(A, v.newDraggableIndex, D);
        return;
      }
      _t(vueExports.unref(r), v.newDraggableIndex, D);
    }
  }
  function b(v) {
    ...
    if (vueExports.isRef(r)) {
      const F = [...vueExports.unref(r)];
      r.value = Dt(F, _);
      return;
    }
    Dt(vueExports.unref(r), _);
  }
  function w(v) {
    if (f) {
      f(v);
    ...
    if (Ke(A), Tt(D, A, I), vueExports.isRef(r)) {
      const M = [...vueExports.unref(r)];
      r.value = St(
        M,
        _,
        x
  ...
  function R(v) {
    const D = vueExports.unref(o);
    return v || (v = En(D) ? Sn(D, e == null ? void 0 : e.$el) : D), v && !Tn(v) && (v = v.$el), v || vn("Root element not found"), v;
  }
  function j() {
    var I;
    const _ = (I = vueExports.unref(i)) != null ? I : {}, { immediate: v, clone: D } = _, A = $e(_, ["immediate", "clone"]);
    return Ct(A, (x, M) => {
      Cn(x) && (A[x] = (F, ...ee) => {
        const Ee = oo();
...
const ft = [
  ...
  setup(t, { slots: e, emit: n, expose: o, attrs: r }) {
    ...
    }, {}), a = vueExports.computed(() => {
      const y = vueExports.toRefs(t), { modelValue: d } = y, f = $e(y, ["modelValue"]), m = Object.entries(f).reduce((b, [w, L]) => {
        const G = vueExports.unref(L);
    ...
    }), s = vueExports.ref(), u = vueExports.reactive(
      on(t.target || s, l, a)
    );
    return o(u), () => {
      var d;
      return vueExports.h(t.tag || "div", { ref: s }, (d = e == null ? void 0 : e.default) == null ? void 0 : d.call(e, u));
    };
  }
});

export { lo as l };
//# sourceMappingURL=vue-draggable-plus.mjs.map
